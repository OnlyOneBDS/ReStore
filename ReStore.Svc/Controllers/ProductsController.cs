...
namespace ReStore.Svc.Controllers
{
  public class ProductsController : BaseApiController
  {
    ...
    {
      var brands = await _context.Products.Select(p => p.Brand).Distinct().OrderBy(p => p).ToListAsync();
      var types = await _context.Products.Select(p => p.Type).Distinct().OrderBy(p => p).ToListAsync();
