...
export default function Catalog() {
  ...
  const { filtersLoaded, productsLoaded, brands, types, productParams, metaData } = useAppSelector(state => state.catalog);
  ...
  return (
    <Grid container columnSpacing={4}>
      <Grid item xs={3}>
        ...
        <Paper sx={{ mb: 2, p: 2 }}>
          <RadionButtonGroup selectedValue={productParams.orderBy} options={sortOptions} onChange={(e) => dispatch(setProductParams({ orderBy: e.target.value }))} />
        </Paper>
        <Paper sx={{ mb: 2, p: 2 }}>
